...
if [ "$api" = true ]; then
	...
	if [[ -v HTB_API_TOKEN ]]; then
		...
		echo "8. Go back to your terminal and execute ${BLUE}echo \$0${DC}"
		if [ "$shell_name" = "bash" ]; then
			echo "9. Execute ${BLUE}echo \"export HTB_API_TOKEN=${GREEN}token_here${BLUE}\" >> ~/.bashrc && source ~/.bashrc${DC}"
		elif [ "$shell_name" = "zsh" ]; then
			echo "9. Execute ${BLUE}echo \"export HTB_API_TOKEN=${GREEN}token_here${BLUE}\" >> ~/.zshrc && source ~/.zshrc${DC}"
		else
			# User has a different shell
			echo "9. Execute ${BLUE}export HTB_API_TOKEN=\"${GREEN}token_here${BLUE}\"${DC}"
		fi
